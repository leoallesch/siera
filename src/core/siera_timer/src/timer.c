...
int siera_timer_start(siera_timer_mgr_t *mgr, siera_timer_t *t,
                       siera_timer_cb_fn cb, void *ctx,
                       uint32_t interval_ms, bool repeat) {
    if (!mgr || !t || !cb) return -1;
    ...
    t->repeat      = repeat;
    t->expiry_ms   = siera_timesource_get_ticks(mgr->timesource) + interval_ms;
    t->active      = true;
...
int siera_timer_restart(siera_timer_mgr_t *mgr, siera_timer_t *t) {
    if (!mgr || !t) return -1;
    t->expiry_ms = siera_timesource_get_ticks(mgr->timesource) + t->interval_ms;
    if (!t->active) {
        t->active = true;
        siera_slist_push(&mgr->timers, &t->node);
    }
...
void siera_timer_tick(siera_timer_mgr_t *mgr) {
    if (!mgr || !mgr->timesource) return;
    uint32_t now = siera_timesource_get_ticks(mgr->timesource);
    ...
    SIERA_SLIST_FOR_EACH_SAFE(&mgr->timers, node, tmp) {
        ...
        if ((int32_t)(now - t->expiry_ms) < 0) continue;
